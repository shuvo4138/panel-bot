...
async def verify_cookie(phpsessid: str) -> bool:
    ...
    try:
        # Step 1: agent/ dashboard page এ GET করো
        ...
        # Redirect to signin = invalid
        if res.status_code in (301, 302):
            loc = res.headers.get("location", "")
            if "signin" in loc.lower() or loc.endswith("/ints") or loc.endswith("/ints/"):
                logger.warning(f"Cookie verify: redirected to {loc} — invalid")
                return False
            # Redirect কিন্তু signin না — accept করো
            return True

        if res.status_code == 200:
            text = res.text.lower()
            # signin page এ চলে গেছে = invalid
            if "account login" in text or 'action=\'signin\'' in res.text:
                return False
            # Dashboard page = valid
            return True
