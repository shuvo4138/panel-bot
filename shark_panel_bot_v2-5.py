...
async def verify_cookie(phpsessid: str) -> bool:
    """Cookie সরাসরি accept করো — verification skip।"""
    return bool(phpsessid and len(phpsessid) > 8)

async def extract_sesskey(phpsessid: str) -> str:
    """Dashboard থেকে sesskey auto-extract করো।"""
    try:
        async with httpx.AsyncClient(
            ...
            res = await client.get(f"{SHARK_BASE_URL}/agent/SMSDashboard")
        logger.info(f"Dashboard status: {res.status_code}, preview: {res.text[:150]}")
        pattern = r"sesskey[='\"\s:]+" + r"([A-Za-z0-9+/=]{10,})"
        sk_match = re.search(pattern, res.text)
        if sk_match:
            logger.info(f"Sesskey found!")
            return sk_match.group(1)
        # Log more HTML to find sesskey pattern
        import re as _re
        # Try many patterns
        all_patterns = [
            r'sesskey[\s]*=[\s]*["\'](\w+)["\'\s]',
            r'sesskey[\s]*:[\s]*["\'](\w+)["\'\s]',
            r'name=["\'"]sesskey["\'"]\s+value=["\'"]([\w]+)["\'"]',
            r'value=["\'"]([\w]+)["\'"]\s+name=["\'"]sesskey["\'"]',
            r'data-sesskey=["\'"]([\w]+)["\'"]',
            r'"sesskey"\s*:\s*"([\w]+)"',
        ]
        for pat in all_patterns:
            m2 = _re.search(pat, res.text, _re.IGNORECASE)
            if m2:
                logger.info(f"Alt pattern matched! sesskey={m2.group(1)[:10]}")
                return m2.group(1)
        # Log 500 chars to see what's in HTML
        logger.warning(f"Sesskey not found. JS section: {res.text[2000:3500]}")
    except Exception as e:
        logger.warning(f"extract_sesskey error: {e}")
    return ""
...
async def shark_fetch_otps(fnum: str = "") -> list:
    ...
    try:
        ...
        async with httpx.AsyncClient(
            ...
            headers={
                "Referer": f"{SHARK_BASE_URL}/agent/SMSDashboard",
        ...
        if "direct script access not allowed" in res.text.lower():
            logger.error(f"Panel blocked! Response: {res.text[:200]}")
            return []

        if not res.text.strip():
            logger.error("Panel returned empty response!")
            return []

        if res.text.strip()[0] not in ('{', '['):
            logger.error(f"Panel non-JSON response: {res.text[:300]}")
            return []
        ...
        results = []
        # Rate limit check
        if rows and isinstance(rows[0], list) and rows[0] and "interval" in str(rows[0][0]).lower():
            logger.warning(f"Panel rate limit: {rows[0][0][:80]}")
            return []
        for row in rows:
...
async def keep_alive_ping() -> bool:
    """Panel dashboard এ lightweight GET পাঠিয়ে session জীবিত রাখো।"""
    ...
    try:
        async with httpx.AsyncClient(
            ...
            res = await client.get(f"{SHARK_BASE_URL}/agent/")
        if res.status_code in (301, 302):
            loc = res.headers.get("location", "")
            if "signin" in loc.lower():
                logger.warning("Keep-alive: session expired!")
                return False
...
async def poll_loop(bot: Bot):
    ...
    _poll_running = True
    _ping_counter = 0
    KEEPALIVE_EVERY = max(1, 120 // max(_settings["poll_interval"], 1))
    ...
    while True:
        try:
            ...
            # Keep-alive ping — প্রতি ~১ মিনিটে একবার
            _ping_counter += 1
...
async def cmd_setcookie(update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if valid:
        # sesskey auto-extract করো dashboard থেকে
        extracted_sesskey = await extract_sesskey(phpsessid) or new_sesskey
        ...
        sk_display = f"`{extracted_sesskey[:16]}...`" if len(extracted_sesskey) > 16 else f"`{extracted_sesskey or 'Auto'}`"
        await msg.edit_text(
